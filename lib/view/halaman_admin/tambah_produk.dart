...
class _TambahProdukState extends State<TambahProduk> {
  ...
  final TextEditingController kategoriProdukController =
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              /// NAMA PRODUK
              textField("Nama Produk *", "Contoh: Buku Tulis Sidu Isi 38",namaProdukController),

              /// SKU
              textField("Kode SKU *", "Contoh: ATK-001", kodeSkuController),

              /// KATEGORI
              textField("Kategori Produk", "Alat Tulis Kantor (ATK)",kategoriProdukController),

              /// HARGA + STOK
              Row(
                children: [
                  Expanded(child: textField("Harga Jual", "RP 0", hargaJualController)),

                  const SizedBox(width: 10),

                  Expanded(child: textField("Jumlah Stok", "0 Unit",jumlahStokController)),
                ],
              ),
                  ...
                  onPressed: () async {
                    if (namaProdukController.text.isEmpty) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(content: Text("Nama Produk belum di isi")),
                      );
                      return;
                    }
                    if (kodeSkuController.text.isEmpty) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(content: Text("Kode SKU belum di isi")),
                      );
                      return;
                    }
                    if (kategoriProdukController.text.isEmpty) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(content: Text("Kategori Produk belum di isi")),
                      );
                      return;
                    }
                    if (hargaJualController.text.isEmpty) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(content: Text("Harga Jual belum di isi")),
                      );
                      return;
                    }
                    if (jumlahStokController.text.isEmpty) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(content: Text("Jumlah Stok belum di isi")),
                      );
                      return;
                    }
